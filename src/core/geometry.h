...
public:
    ...
    // The default version of these constructors are fine for release builds. For debug
    // we define these constructors to add the Assert checks.
...
public:
    ...
    Point3f o;
    Vector3f d;
    mutable Float tMax; // Can modify this value in const functions
    Float time;
    const Medium *medium;
};
...
inline bool Bounds3<T>::IntersectP(const Ray &ray, Float *hitt0, Float *hitt1) const {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        // Update parametric interval from slab intersection $t$ value
